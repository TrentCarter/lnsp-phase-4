#!/usr/bin/env python3
"""
LSTM Baseline Trainer for Vector-Native LVM
============================================

Trains a simple LSTM to predict next vector from context (autoregressive).

Architecture:
- Input: sequence of 768D vectors (context window)
- LSTM layers: 2 layers, hidden_dim=512
- Output: next 768D vector prediction
- Loss: MSE between predicted and target vectors

Training Data:
- artifacts/lvm/training_sequences_ctx5.npz
- 42,108 training pairs with context_length=5
"""
...
class LSTMVectorPredictor(nn.Module):
    """LSTM model for next-vector prediction"""
...
def train_epoch(
    ...
    *,
    ...
    """Train for one epoch with optional regularizers."""
    ...
    for batch_idx, (contexts, targets) in enumerate(dataloader):
        ...
        if batch_idx % 100 == 0:
            info_val = stats.get("loss_info", 0.0)
            print(
...
def main():
    ...
    parser.add_argument('--output-dir', default='artifacts/lvm/models/lstm_baseline')
    ...
    for epoch in range(args.epochs):
        ...
        history.append({
            ...
            'train_loss_info': train_stats['loss_info'],
            'train_loss_moment': train_stats['loss_moment'],
            'train_loss_variance': train_stats['loss_variance'],
            'val_loss': val_loss,
