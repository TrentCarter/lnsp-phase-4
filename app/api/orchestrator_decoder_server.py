#!/usr/bin/env python3
"""
Orchestrator-based Vec2Text Decoder Service (Port 7002) - OPTIMIZED
Fast in-memory decoding using pre-loaded Vec2Text models (no subprocess spawning)

This service uses Vec2TextProcessor for decoding, keeping models warm in memory.
ONLY use vectors from port 7001. DO NOT use vectors from port 8767.
"""
...
# Decoder configurations (CPU for stability, can use MPS if needed)
decoder_configs = {
    'jxe': {
        'teacher_model': 'sentence-transformers/gtr-t5-base',
        'device': 'cpu',  # Can change to 'mps' for Apple Silicon
        'random_seed': 42
    },
    'ielab': {
        'teacher_model': 'sentence-transformers/gtr-t5-base',
...
async def lifespan(app: FastAPI):
    ...
    logger.info("🚀 Starting Vec2Text Decoder (Port 7002)...")
    try:
        await load_vec2text_processors()
        logger.info("✅ Orchestrator decoder ready (in-memory mode)")
...
app = FastAPI(
    title="Orchestrator Decoder Service",
    version="2.0.0",
    lifespan=lifespan
)
...
class DecodeRequest(BaseModel):
    ...
    steps: Optional[int] = 1  # Default to 1 for speed (web UI defaults to 3)
...
async def load_vec2text_processors():
    ...
    logger.info("Loading vec2text processors...")
    ...
    for decoder_name, config in decoder_configs.items():
        try:
            ...
            logger.info(f"✅ {decoder_name.upper()} processor loaded successfully")
...
def decode_single_vector_in_memory(
    ...
    """Decode a single vector using in-memory processor (FAST - no subprocess)"""
...
async def decode(request: DecodeRequest):
    """
    Decode 768D vectors to text using in-memory Vec2Text processors (FAST!)

    These vectors MUST come from port 7001 encoder.
    ...
    try:
        ...
        logger.info(f"Decoding {len(request.vectors)} vectors with {request.subscriber} (steps={request.steps})...")
        ...
        logger.info(f"✓ Decoded {len(decoded_texts)} texts in {elapsed_ms}ms")
