...
Uses Gemini 2.5 Flash (or fallback) to decompose DevSecOps lane job cards into Manager tasks

Contract: Based on docs/contracts/DIRECTOR_DEVSECOPS_SYSTEM_PROMPT.md
"""
import os
import json
...
class ManagerTaskDecomposer:
    ...
    - Manager-level CI/CD task breakdown
    - Test requirements
    ...
    def __init__(self):
        ...
        self.llm_provider = os.getenv("DIR_DEVSECOPS_LLM_PROVIDER", "google")
        self.llm_model = os.getenv("DIR_DEVSECOPS_LLM", "gemini-2.5-flash")
        self.llm_endpoint = self._get_llm_endpoint()

    def _get_llm_endpoint(self) -> str:
        """Get LLM endpoint based on provider"""
        if self.llm_provider == "google":
            return "https://generativelanguage.googleapis.com/v1beta/models/" + self.llm_model + ":generateContent"
    ...
    def _build_decomposition_prompt(
        ...
        return f"""You are Dir-DevSecOps, the Director of the DevSecOps lane in a Polyglot Agent Swarm (PAS).

Your role is to decompose DevSecOps lane job cards into Manager-level CI/CD tasks. Each Manager will coordinate 1-5 Programmers to complete their task.
...
Decompose this job card into Manager-level CI/CD tasks. Follow these rules:
