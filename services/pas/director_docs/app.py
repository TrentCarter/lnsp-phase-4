#!/usr/bin/env python3
"""
Director-Code Service - Docs Lane Coordinator
...
- Monitor Managers
- Validate acceptance gates (documentation, completeness, reviews)
- Report to Architect
...
from services.pas.director_docs.decomposer import ManagerTaskDecomposer
...
app = FastAPI(title="Director-Docs", version="1.0.0")
...
heartbeat_monitor.register_agent(
    agent="Dir-Docs",
    parent="Architect",
    llm_model=os.getenv("DIR_DOCS_LLM", "anthropic/claude-sonnet-4-5"),
    role="director",
    tier="coordinator"
)
...
async def report_to_architect(run_id: str, lane_report: LaneReport):
    ...
    try:
        ...
        logger.log_response(
            ...
            run_id=run_id,
            status=lane_report.state,
            metadata={"lane": "Docs", "rpc_error": str(e)}
        )
...
async def receive_agent_message(
    ...
    This is the RECOMMENDED way for Architect to communicate with Dir-Docs.
    Enables bidirectional Q&A, status updates, and context preservation.

    Flow:
    - Architect creates thread with delegation message
    - Dir-Docs receives via this endpoint
    - Dir-Docs can ask questions using agent_chat.send_message()
    - Dir-Docs sends status updates during execution
    - Dir-Docs closes thread on completion/error
...
async def process_agent_chat_message(request: AgentChatMessage):
    ...
    try:
        ...
        system_prompt = get_system_prompt_with_ask_parent(
            ...
            role_description="Docs Lane Director responsible for documentation, completeness, and reviews"
        )
        ...
        llm_response = await call_llm_with_tools(
            ...
            model=os.getenv("DIR_DOCS_LLM", "anthropic/claude-sonnet-4-5")
        )
        ...
        job_card_id = f"job-{run_id}-docs-{uuid.uuid4().hex[:8]}"
    ...
    try:
        ...
        if all(validation_results.values()):
            ...
            await agent_chat.send_message(
                ...
                content=f"✅ Docs lane tasks completed successfully. All Manager tasks passed validation."
            )
            ...
            await agent_chat.send_message(
                ...
                to_agent="Architect",
                message_type="error",
                content=f"❌ Docs lane tasks failed. Failed managers: {', '.join(failed_managers)}"
            )
...
async def register_manager(manager_id: str, endpoint: str):
    ...
    This enables Dir-Docs to submit tasks directly via RPC
