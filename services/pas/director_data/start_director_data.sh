...
if [ "$DIR_DATA_LLM_PROVIDER" = "anthropic" ] && [ -z "${ANTHROPIC_API_KEY:-}" ]; then
...
echo "=== Starting Director-Data Service ==="
echo "Port: 6113"
echo "LLM Provider: $DIR_DATA_LLM_PROVIDER"
echo "LLM Model: $DIR_DATA_LLM"
echo "Architect URL: $ARCHITECT_URL"
echo ""

# Start service
exec python -m uvicorn services.pas.director_data.app:app \
    --host 127.0.0.1 \
    --port 6113 \
    --reload
