#!/usr/bin/env python3
"""
Director-Code Service - Data Lane Coordinator
...
- Monitor Managers
- Validate acceptance gates (ingestion, validation, schema checks)
- Report to Architect
...
# Director-Data specific imports
...
async def report_to_architect(run_id: str, lane_report: LaneReport):
    ...
    try:
        ...
        logger.log_response(
            ...
            run_id=run_id,
            status=lane_report.state,
            metadata={"lane": "Data", "rpc_error": str(e)}
        )
...
async def receive_agent_message(
    ...
    This is the RECOMMENDED way for Architect to communicate with Dir-Data.
    Enables bidirectional Q&A, status updates, and context preservation.

    Flow:
    - Architect creates thread with delegation message
    - Dir-Data receives via this endpoint
    - Dir-Data can ask questions using agent_chat.send_message()
    - Dir-Data sends status updates during execution
    - Dir-Data closes thread on completion/error
...
async def process_agent_chat_message(request: AgentChatMessage):
    ...
    try:
        ...
        system_prompt = get_system_prompt_with_ask_parent(
            ...
            role_description="Data Lane Director responsible for data ingestion, validation, and schema checks"
        )
        ...
        llm_response = await call_llm_with_tools(
            ...
            model=os.getenv("DIR_DATA_LLM", "anthropic/claude-sonnet-4-5")
        )
    ...
    try:
        ...
        if all(validation_results.values()):
            ...
            await agent_chat.send_message(
                ...
                content=f"✅ Data lane tasks completed successfully. All Manager tasks passed validation."
            )
            ...
            await agent_chat.send_message(
                ...
                to_agent="Architect",
                message_type="error",
                content=f"❌ Data lane tasks failed. Failed managers: {', '.join(failed_managers)}"
            )
...
async def register_manager(manager_id: str, endpoint: str):
    ...
    This enables Dir-Data to submit tasks directly via RPC
