#!/usr/bin/env python3
"""
Manager-Models-01 Service - Code Lane Task Breakdown
...
app = FastAPI(title="Manager-Models-01", version="1.0.0")
...
SERVICE_NAME = "Manager-Models-01"
SERVICE_PORT = 6144
...
PRIMARY_LLM = os.getenv("MGR_MODELS_01_PRIMARY_LLM", "google/gemini-2.5-flash")
BACKUP_LLM = os.getenv("MGR_MODELS_01_BACKUP_LLM", "anthropic/claude-haiku-4")
...
async def handle_incoming_message(message: AgentChatMessage):
    """
    Handle incoming messages from parent (Dir-Models) or children.

    Called automatically by the message poller when new messages arrive.
    """
    print(f"[{{get_agent_id() if 'get_agent_id' in dir() else 'Mgr-Models-01'}}] Received {{message.message_type}} from {{message.from_agent}}")

    if message.message_type == "delegation":
        # Handle delegation from parent
        print(f"[{{get_agent_id() if 'get_agent_id' in dir() else 'Mgr-Models-01'}}] Delegation: {{message.content}}")
        # TODO: Process delegation

    elif message.message_type == "question":
        # Handle question from child
        print(f"[{{get_agent_id() if 'get_agent_id' in dir() else 'Mgr-Models-01'}}] Question: {{message.content}}")
        # TODO: Answer question

    elif message.message_type == "status":
        # Handle status update
        print(f"[{{get_agent_id() if 'get_agent_id' in dir() else 'Mgr-Models-01'}}] Status: {{message.content}}")

    elif message.message_type == "completion":
        # Handle completion
        print(f"[{{get_agent_id() if 'get_agent_id' in dir() else 'Mgr-Models-01'}}] Completion: {{message.content}}")

    elif message.message_type == "error":
        # Handle error
        print(f"[{{get_agent_id() if 'get_agent_id' in dir() else 'Mgr-Models-01'}}] Error: {{message.content}}")
...
_agent_id_for_chat = "Mgr-Models-01"
add_agent_chat_routes(
    app=app,
