...
WITH article_stats AS (
    SELECT
        chunk_position->>'article_index' AS article_id,
    ...
    FROM cpe_entry
    WHERE dataset_source = 'wikipedia'
...
selected_articles AS (
    ...
    (SELECT * FROM article_stats WHERE chunk_count > 100 LIMIT 1)  -- Very long
    UNION ALL
    (SELECT * FROM article_stats WHERE chunk_count BETWEEN 50 AND 100 LIMIT 1)  -- Long
    UNION ALL
    (SELECT * FROM article_stats WHERE chunk_count BETWEEN 20 AND 50 LIMIT 1)  -- Medium
    UNION ALL
    (SELECT * FROM article_stats WHERE chunk_count BETWEEN 10 AND 20 LIMIT 1)  -- Short
)
SELECT
    sa.article_id,
    sa.title,
    sa.chunk_count,
    ce.chunk_position->>'chunk_index' AS chunk_index,
    ce.concept_text,
    ce.source_chunk,
    ce.cpe_id,
    ce.created_at
...
JOIN cpe_entry ce ON
    ce.chunk_position->>'article_index' = sa.article_id
    AND ce.dataset_source = 'wikipedia'
ORDER BY
    sa.chunk_count DESC,
    sa.article_id,
