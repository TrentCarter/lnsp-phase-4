...
def insert_entry(conn, core: dict):
    ...
    INSERT INTO cpe_entry (cpe_id, mission_text, source_chunk, concept_text,
        probe_question, expected_answer, soft_negatives, hard_negatives,
    ...
    VALUES (%(cpe_id)s, %(mission_text)s, %(source_chunk)s, %(concept_text)s,
        %(probe_question)s, %(expected_answer)s, %(soft_negatives)s, %(hard_negatives)s,
        %(domain_code)s, %(task_code)s,
...
def upsert_vectors(conn, cpe_id: str, fused_vec, question_vec, concept_vec, tmd_dense=None, fused_norm=None):
    cur = conn.cursor()
    sql = """
    ...
    """
    tmd_list = tmd_dense.tolist() if tmd_dense is not None else None
    cur.execute(sql, (cpe_id, fused_vec.tolist(), question_vec.tolist(), concept_vec.tolist(), tmd_list, fused_norm))
    conn.commit()
    cur.close()
...
class PostgresDB:
    ...
    def insert_cpe(self, cpe_record: dict) -> bool:
        ...
        payload["soft_negatives"] = json.dumps(payload.get("soft_negatives", []))
        payload["hard_negatives"] = json.dumps(payload.get("hard_negatives", []))
        result = bool(insert_entry(self.conn, payload))

        # Also insert vectors into cpe_vectors table
        if result and cpe_record.get("fused_vec") and cpe_record.get("concept_vec") and cpe_record.get("question_vec"):
            import numpy as np
            fused_vec = np.array(cpe_record["fused_vec"], dtype=np.float32)
            concept_vec = np.array(cpe_record["concept_vec"], dtype=np.float32)
            question_vec = np.array(cpe_record["question_vec"], dtype=np.float32)
            tmd_dense = np.array(cpe_record["tmd_dense"], dtype=np.float32) if cpe_record.get("tmd_dense") else None
            fused_norm = cpe_record.get("fused_norm")
            upsert_vectors(self.conn, cpe_record["cpe_id"], fused_vec, question_vec, concept_vec, tmd_dense, fused_norm)
